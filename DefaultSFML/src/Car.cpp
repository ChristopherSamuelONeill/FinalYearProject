...
void Car::update(float dt)
{
	if (m_bSpawned)
	{
		...
		{
			//car has reach destination or crashed
...
void Car::CheckForCar(Car & car)
{
	if (m_bSpawned)
	{
		...
		if (collisionCheck(m_sfCarRect, car.m_sfCarRect) && (m_bSpawned == true && car.m_bSpawned == true))
		{
			// crash
			m_bCrashed = true;
		}
